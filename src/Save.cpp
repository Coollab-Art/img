...
void save_png(std::filesystem::path file_path,
              Size::DataType        width,
...
void save_jpeg(std::filesystem::path file_path, const Image& image, bool flip_vertically)
{
    save_jpeg(file_path.string().c_str(), image.width(), image.height(), image.data(), image.channels_count(), flip_vertically);
}

void save_jpeg(const char*    file_path,
               Size::DataType width,
               Size::DataType height,
               const void*    data,
               int            channels_count,
...
{
    stbi_flip_vertically_on_write(flip_vertically ? 1 : 0);
    stbi_write_jpg(file_path, static_cast<int>(width), static_cast<int>(height), channels_count, data, 100);
}
